netVisual_bubble_new <- function (object, sources.use = NULL, targets.use = NULL, signaling = NULL, 
          ...
          angle.x = 90, vjust.x = NULL, hjust.x = NULL, return.data = FALSE) 
{
  ...
  if (is.null(comparison)) {
    ...
    df.net$pval[df.net$pval > 0.01] = 1
    df.net$pval[df.net$pval > 0.001 & df.net$pval <= 
                  0.01] = 2
    df.net$pval[df.net$pval <= 0.001] = 3
  ...
  g <- ggplot(df, aes(x = paste0(source,' -> ',target), y = interaction_name_2, 
  ...
  values <- c(1, 2, 3)
  names(values) <- c("p > 0.01", "0.001 < p < 0.01", "p < 0.001")
